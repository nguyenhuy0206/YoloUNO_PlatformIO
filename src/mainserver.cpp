  ...
  //   html.replace("{{LED1}}", led1);
  //   html.replace("{{LED2}}", led2);
  ...
  void setupServer() {
    ...
    server.on("/dashboard.html", HTTP_GET, [](){ serveFile("/dashboard.html"); });
    server.on("/control.html",   HTTP_GET, [](){ serveFile("/control.html");   });
    server.on("/cloud.html",     HTTP_GET, [](){ serveFile("/cloud.html");     });
    server.on("/system_log.html",HTTP_GET, [](){ serveFile("/system_log.html");});
    server.on("/wifi.html",      HTTP_GET, [](){ serveFile("/wifi.html");      });
    server.on("/login.html",     HTTP_GET, [](){ serveFile("/login.html");     });
    ...
    server.on("/humidity.json",   HTTP_GET, [](){ serveFile("/humidity.json");  });
    server.on("/Thermometer.json",HTTP_GET, [](){ serveFile("/Thermometer.json"); });
  ...
  void main_server_task(void *pvParameters){
    ...
    if (!SPIFFS.begin(true)) {     // true = format nếu mount fail (tùy bạn)
